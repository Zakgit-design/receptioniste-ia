...
### Tâche #73 — RendezVous/ClientFinal réels + établissement fiable des appels (2026-07-17)

**Préalable, même nature qu'à la tâche #71 :** aucune ligne `Service` n'existait encore en base pour Barber Concept, alors que `RendezVous.serviceId` est obligatoire. Script ponctuel commité `scripts/seed-services-barber-concept.js` (15 services de base, tarifs/durées tirés de `src/prompts/system-prompt.md`). **Limite honnête assumée, pas résolue :** les tarifs étudiants et les prestations propres à un salon (Coupe Henok à Rive, locks/twists aux Eaux-Vives) ne sont pas seedés — le schéma `Service` n'a qu'un prix unique par service, pas de variation par salon/profil client. Une réservation réelle pour l'une de ces prestations spécifiques ne trouve donc pas de correspondance (le webhook journalise un avertissement plutôt que d'inventer une correspondance).

**Découverte empirique importante, contredisant une hypothèse du Sprint 3 :** en inspectant via l'API Vapi 10 vrais appels déjà passés depuis le 2026-07-15, l'outil natif `google_calendar_tool` s'est révélé n'avoir aucun paramètre séparé pour le salon/la prestation/le client — seulement un `summary` en texte libre, dont le format varie fortement d'un appel à l'autre (ordre différent, séparateurs différents, salon parfois absent). L'hypothèse du journal Sprint 3 (« Coupe classique — Marie Dupont (Cornavin) », un format fixe supposé) ne s'est donc pas confirmée. Résolution adaptée en conséquence (`src/call-webhook.js`, fonction `resoudreReservation`) : rapprochement par présence d'un nom **connu** (catalogues fermés : les 6 salons réels, les 15 services réels) dans ce texte libre, jamais une correspondance approximative devinée — si le salon ou la prestation ne peut pas être identifié avec confiance, aucun `RendezVous` n'est créé, un avertissement explicite est journalisé, et `Appel.etablissementId` reste `null` (« non déterminé »), conforme à la décision actée dans `docs/architecture.md`.

**Logique implémentée** (même webhook que la tâche #72, `src/call-webhook.js`) : quand une réservation a réellement abouti, crée/retrouve le `ClientFinal` par téléphone (met à jour son nom si découvert et absent), crée le `RendezVous` (établissement, service, durée du service, id Google Calendar réel), puis met à jour la ligne `Appels` de la tâche #72 (`rendez_vous_id`, et surtout **`etablissement_id` = celui du RDV réel**, la vraie source de vérité). Toute cette résolution est isolée dans son propre bloc d'erreur : un échec ne peut jamais empêcher l'écriture `Appels`/`Conversations` déjà acquise depuis la tâche #72.

**Écart de méthode assumé, signalé explicitement plutôt que dissimulé :** contrairement à la tâche #72 (validée par un vrai appel téléphonique), cette tâche a été vérifiée en rejouant directement, à travers le code déployé, les payloads de fin d'appel de vrais appels déjà passés sur l'assistant de production (rapprochement complet réussi sur un cas réel « Rive » + « Coupe classique » + client extrait correctement ; cas sans salon reconnu → non créé comme prévu ; cas prestation Henok non seedée → non créé comme prévu ; rejeu du même appel → doublon ignoré, idempotence confirmée). Aucun nouvel appel téléphonique de test n'a été passé pour cette tâche précise — à garder en tête, un vrai test oral de bout en bout avec réservation reste recommandé (voir tâche #75).

**Bug de scope multi-tenant trouvé et corrigé en testant** (`dashboard/src/lib/scope-client.ts`, nouvelle fonction `getAgentIdsEntreprise`) : le code Sprint 6 ne retrouvait les appels d'un utilisateur qu'à travers l'établissement de l'agent (`agentIA.etablissementId`, fixe/arbitraire depuis la tâche #71) — un responsable d'établissement autre que celui de l'agent (Cornavin) ne voyait donc **aucun appel de son propre salon**. Corrigé : le scope "quels appels m'appartiennent" se fait désormais par les agents de mon entreprise (indépendant de l'établissement de l'agent), avec une restriction supplémentaire sur `Appel.etablissementId` appliquée uniquement pour un responsable d'établissement (qui ne doit voir ni un autre établissement, ni un appel « non déterminé », potentiellement d'un autre salon). Fichiers mis à jour en conséquence : `dashboard/src/app/(client)/app/appels/data.ts`, `.../app/data.ts` (Vue d'ensemble), `.../app/etablissements/data.ts`, `dashboard/src/lib/appels-client.ts`. Vérifié par requêtes directes contre la vraie base : propriétaire voit tout (y compris « non déterminés »), responsable scopé à Rive ne voit que Rive, responsable scopé à Cornavin n'en voit aucun (correct, comportement attendu vu les données de test utilisées).

**Déploiement réel :** code poussé sur `origin/main`, Render redéployé (mécanisme établi depuis le Sprint 4), `GET /health` reconfirmé 200 par la session principale après déploiement.

Vérifié (session principale, après le rapport de l'agent) : base repartie propre (0 ligne `appels`/`rendez_vous`/`clients_finaux`, 6 établissements + 15 services réels conservés) ; `dashboard` build/lint/27 tests au vert ; code de scope relu et jugé correct (isolation par agents d'entreprise + restriction établissement en défense en profondeur pour un responsable).
...
**Décision initiale du fondateur, à la suite d'un cadrage stratégique de type CTO :** plutôt que d'enchaîner sur les sprints déjà prévus (Get Time, transfert humain, polish), la priorité immédiate devient double :
1. **Préparer un vrai déploiement chez Barber Concept** — au-delà de la démo déjà testée.
...
**Raisonnement clé retenu :** ces deux objectifs partagent le même verrou technique — un mécanisme générique de provisioning par établissement (numéro + assistant + agenda + outils) résout à terme le partage actuel d'un seul numéro/agenda entre les 6 salons Barber Concept **et** sert de base pour onboarder toute nouvelle entreprise. Ce raisonnement reste valable, mais son **calendrier** a été corrigé le jour même (voir ci-dessous) avant que le développement ne démarre.

**Correction du même jour (fondateur), avant tout code :** Barber Concept n'a pas encore confirmé le pilote — l'accueil d'Henok est positif mais la décision appartient encore à ses associés. Construire immédiatement un numéro/assistant/agenda pour chacun des 6 salons serait donc une sur-ingénierie prématurée, engagée avant toute confirmation commerciale. **Le Sprint 7 est recadré autour d'un pilote réel limité à un seul salon** (numéro, assistant, agenda et établissement uniques — Jonction, choisi par le fondateur le 2026-07-22, voir tâche #76 ci-dessous), avec un objectif concret et vendable : Henok doit pouvoir se connecter au Dashboard Client depuis son propre ordinateur ou téléphone. Les 5 autres salons restent présents en base, non configurés, non actifs. Le mécanisme générique multi-établissements, l'onboarding d'un 2e client, l'infra toujours active et le catalogue de services généralisé sont conservés comme **phase suivante, déclenchée par la validation commerciale de Barber Concept** — pas construits maintenant.

**Roadmap réorganisée en conséquence** (`docs/roadmap.md`) :
- Sprint 7 — Pilote réel mono-salon Barber Concept : établissement unique, Dashboard déployé publiquement sur Vercel, parcours d'invitation Clerk pour Henok (tâches #76-81)
- Phase suivante (en pause, non détaillée davantage pour l'instant) : extension multi-salons, Sprint 8 (catalogue de services), Sprint 9 (infra toujours active, tâches #47-49), Sprint 10 (onboarding industrialisé), Sprint 11 (supervision minimale) — tous déclenchés par la validation commerciale de Barber Concept, pas par le calendrier des sprints

**Volontairement laissé de côté pour l'instant, pas oublié :** la facturation réelle (Stripe) reste hors périmètre tant qu'aucune relation commerciale formelle n'est signée avec Barber Concept ou un futur client — à réintroduire dès qu'un accord commercial se concrétise. De même, aucun hébergement payant n'est souscrit tant que le pilote n'est pas confirmé — Render gratuit reste accepté pour le backend, limite documentée dans `docs/architecture.md`.

### Tâche #76 — Établissement pilote confirmé : Jonction (2026-07-22)

Le fondateur a tranché pour **Jonction** plutôt que Cornavin (proposé par défaut car déjà configuré depuis le Sprint 6bis, tâche #71).

**État réel vérifié en base avant tout changement :** un seul agent IA existe (numéro `+41225391668`, assistant Vapi `c08b8b99-...`), rattaché à Cornavin. Aucun changement Twilio/Vapi requis pour ce changement — seule la colonne `agents_ia.etablissement_id` détermine quel établissement s'affiche comme "configuré" dans le Dashboard Client (voir `dashboard/src/app/(client)/app/etablissements/data.ts`, `etablissement.agentsIA[0]`), le numéro et l'assistant restant les mêmes.

Script ponctuel commité `scripts/set-pilot-etablissement.js` (même méthode que les scripts précédents, tâches #71/#73) : relit l'état avant/après, met à jour `agents_ia.etablissement_id` vers l'id de Jonction. Exécuté avec succès, vérifié (avant : Cornavin, après : Jonction, même ligne `agents_ia`, même numéro, même assistant).

**Vérification du code des dashboards demandée par la tâche :** recherche de toute mention "Cornavin" dans le code — aucune logique en dur trouvée en dehors des données de démonstration fictives du Dashboard Administrateur (`(dashboard)/appels/data.ts`, `(dashboard)/entreprises/data.ts`, 3 entreprises fictives sans lien avec Barber Concept, `estDemo: true`). Le code réel du Dashboard Client est déjà entièrement piloté par la donnée (`etablissement.agentsIA[0]`) — un seul commentaire obsolète mentionnant Cornavin a été corrigé. `npm run build`/`test` (27 tests) au vert après le changement.
...
### Tâche #77 (suite) — Deuxième incident : échec runtime après le déploiement (2026-07-22)

Le build réparé ci-dessus a permis au déploiement d'aboutir (statut `READY`), mais visiter `/app` après connexion donnait "This page couldn't load – A server error occurred." Le fondateur a installé et authentifié la CLI Vercel dans la foulée (`vercel whoami` → `zakgit-design`), ce qui a permis d'investiguer directement plutôt que par aller-retours de logs copiés-collés.

**Cause exacte trouvée dans les logs runtime réels (`vercel logs`) :** même famille d'erreur qu'au build (`PrismaClientKnownRequestError`, code `P1001`, `DriverAdapterError: DatabaseNotReachable`), mais cette fois sur `prisma.entreprise.findUnique()` — qui n'existe qu'à un seul endroit du code : `src/auth/index.ts`, fonction `getCurrentUser()`, appelée en tout premier par chacune des pages de `(client)/app/*`. `src/proxy.ts` redirige tout utilisateur avec une organisation Clerk active vers `/app` juste après connexion — c'est donc précisément cette route qui plantait, immédiatement après la connexion.

**Cause racine identifiée via la CLI :** `vercel env ls` confirmait `DATABASE_URL` présente en Production, mais `vercel env pull` renvoyait `[SENSITIVE]` au lieu de la valeur réelle — cette variable avait été enregistrée côté Vercel comme variable **"Sensitive"** (type d'option Vercel : valeur illisible après coup, y compris pour le propriétaire du projet, ni depuis le dashboard ni depuis la CLI). Impossible donc de prouver que la valeur stockée était fausse, mais tout aussi impossible de le vérifier — et le correctif de build de la tâche précédente avait justement supprimé la seule vérification qui aurait pu détecter ça plus tôt (avant, un `DATABASE_URL` cassé faisait échouer le build ; maintenant que `/entreprises` ne touche plus la base au build, un `DATABASE_URL` cassé ne se voit qu'au premier vrai appel réseau, au runtime).

**Correction :** valeur réécrite directement depuis `dashboard/.env.local` vers Vercel (`vercel env add DATABASE_URL production --force --no-sensitive`, valeur transmise par `stdin`, jamais affichée en clair dans aucune sortie), pour Production et Preview, avec le flag `--no-sensitive` pour qu'elle redevienne vérifiable à l'avenir. Confirmé identique caractère pour caractère à la valeur locale par un second `vercel env pull` comparé en aveugle (diff, jamais affiché en clair non plus).

**Incident opérationnel trouvé et corrigé en cours de route :** le premier redéploiement (`vercel deploy --prod`) a été lancé depuis la racine du dépôt sans lien Vercel valide à cet endroit (le lien avait été créé dans `dashboard/` lors du tout premier essai) — la CLI a silencieusement créé un nouveau projet fantôme au lieu de prévenir, et déployé le contenu de la racine du dépôt (le backend Express, pas le dashboard) : confirmé par les en-têtes de réponse (`x-powered-by: Express`, "Cannot GET /app") et par `vercel inspect` (build à une seule fonction générique au lieu des dizaines de routes/segments Next.js attendues). Corrigé en supprimant ce lien, puis en relançant `vercel link --project=receptioniste-ia` **depuis la racine du dépôt** (pas depuis `dashboard/`) — cohérent avec le réglage `Root Directory: dashboard` du projet Vercel, qui suppose que le lien vit au niveau du dépôt, pas du sous-dossier. Un deuxième projet fantôme ("dashboard", créé lors du tout premier `vercel link --yes` sans argument) et celui-ci ("receptionniste-ia", orthographe à deux "n" du dossier local, différente du vrai projet "receptioniste-ia") restent visibles dans le compte Vercel, vides, sans déploiement actif — suppression interactive tentée sans succès (la CLI boucle sur la confirmation en mode non interactif), laissée pour un nettoyage manuel par le fondateur, aucun impact fonctionnel.

**Vérification finale :** redéploiement propre depuis la racine correctement liée (`vercel deploy --prod`, build confirmé Next.js via `vercel inspect`), le fondateur s'est reconnecté réellement sur https://receptioniste-ia.vercel.app. Logs runtime revérifiés avec `vercel logs --level error` : **aucun résultat**. Toutes les pages visitées pendant ce test (Dashboard Client : `/app`, `/app/appels`, `/app/rendez-vous`, `/app/etablissements`, `/app/equipe`, `/app/parametres` ; Dashboard Admin : `/`, `/entreprises`, `/entreprises/[id]`, `/appels`, `/finances`, `/sante-plateforme`, `/utilisateurs`) apparaissent en `info` uniquement dans les logs, sans erreur.

**Point de vigilance retenu pour la suite :** toute variable d'environnement sensible ajoutée sur Vercel devrait être vérifiée immédiatement après création (`vercel env pull` puis comparaison) plutôt que supposée correcte — le mode "Sensitive" rend une erreur de copier-coller invisible jusqu'au premier vrai incident utilisateur.
